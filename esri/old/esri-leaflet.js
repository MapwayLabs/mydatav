...
(function (factory) {
    ...
    (function (EsriLeaflet) {
        ...
        // GeoJSON -> ArcGIS
        EsriLeaflet.Util.geojsonToArcGIS = function (geojson, idAttribute, _wkid) {
            idAttribute = idAttribute || 'OBJECTID';
            var spatialReference = _wkid ? { wkid: _wkid, latestWkid: _wkid } : { wkid: 4326 };
            var result = {};
            var i;
            /*
            *修改说明：默认使用坐标系4326，修改为不使用
            *修改时间：2015/8/22
            *注释时间：注释代码为原有代码(勿删)
            */
            switch (geojson.type) {
                ...
                case 'Feature':
                    if (geojson.geometry) {
                        result.geometry = EsriLeaflet.Util.geojsonToArcGIS(geojson.geometry, idAttribute, _wkid);
                    }
                ...
                case 'FeatureCollection':
                    ...
                    for (i = 0; i < geojson.features.length; i++) {
                        result.push(EsriLeaflet.Util.geojsonToArcGIS(geojson.features[i], idAttribute, _wkid));
                    }
                    break;
                case 'GeometryCollection':
                    result = [];
                    for (i = 0; i < geojson.geometries.length; i++) {
                        result.push(EsriLeaflet.Util.geojsonToArcGIS(geojson.geometries[i], idAttribute, _wkid));
                    }
                    break;
            }
        ...
        _request: function (method, path, params, callback, context) {
            ...
            if (this._authenticating) {
                ...
                if ((method === 'get' || method === 'request') && !this.options.useCors) {
                    ...
                    var str = url + serialize(params);
                    if (str.length > 2000) {
                        L.esri.Request.request("library/leaflet/arcgis/proxy.ashx?" + url, params, wrappedCallback, this);
                    } else {
    ...
    EsriLeaflet.Tasks.Task = L.Class.extend({
        ...
        initialize: function (endpoint, options) {
            ...
            if (L.dciMap) { this.params.outSr = L.dciMap.options.crsNum; }
        ...
        _request: function (method, path, params, callback, context) {
            ...
            if ((method === 'get' || method === 'request') && !this.options.useCors) {
                ...
                if (this.options.proxy || serialize(params).length > 2000) {
                    return L.esri.Request.request("proxy/proxy.ashx?" + url, params, callback, context);
                }
    ...
    EsriLeaflet.Tasks.Query = EsriLeaflet.Tasks.Task.extend({
        ...
        _setGeometry: function (geometry) {
            if (L.dciMap) {
                this.params.inSr = L.dciMap.options.crsNum || 4326;
            }
            //this.params.inSr = 4326;
    ...
    EsriLeaflet.Tasks.IdentifyFeatures = EsriLeaflet.Tasks.Identify.extend({
        ...
        on: function (map) {
            ...
            //this.params.sr = this._map.options.crs.code.split(':')[1];
            this.params.sr = this._map.options.crsNum;
            var latlng = L.latLng(extent.ymin, extent.xmin);
            var minPoint = this._map.options.crs.project(latlng);
            latlng = L.latLng(extent.ymax, extent.xmax);
            var maxPoint = this._map.options.crs.project(latlng);
            this.params.mapExtent = ([minPoint.x, minPoint.y, maxPoint.x, maxPoint.y]).join(',');
            return this;
        },

        at: function (latlng) {
            //latlng = L.latLng(latlng);
            var point = this._map.options.crs.project(latlng);
            this.params.geometry = ([point.x, point.y]).join(',');
            this.params.geometryType = 'esriGeometryPoint';
            return this;
        },

        setGeometry: function (geometry, isCrs) { //isCrs,true进行坐标转换，经纬度转平面坐标
            // 判断图形类型
            var processedInput = {
                'geometryType': '',
                'features': []
            ...
            var json = {};
            var _code = this._map.options["crsNum"];

            if (processedInput.geometryType == "esriGeometryPoint") {
                this.params.geometry = this._map.options.crs.project(L.latLng(processedInput.features[0].geometry.x, processedInput.features[0].geometry.y)).x + "," + this._map.options.crs.project(L.latLng(processedInput.features[0].geometry.x, processedInput.features[0].geometry.y)).y; //平面坐标
            } else {
                //json.spatialReference = { wkid: parseInt(code[1]) };
                json.spatialReference = { wkid: isNaN(_code) ? _code : parseInt(_code) };
                if (processedInput.geometryType == "esriGeometryPolyline") {
                    if (isCrs)
                        json.paths = [convertLatLng(processedInput.features[0].geometry.paths[0], this._map)];
                    else {
                        json.paths = processedInput.features[0].geometry.paths[0];
                    }
                } else {
                    if (processedInput.geometryType == "esriGeometryPolygon") {
                        if (isCrs)
                            json.rings = [convertLatLng(processedInput.features[0].geometry.rings[0], this._map)];
                        else {
                            json.rings = processedInput.features[0].geometry.rings;
                        }
                    }
                }
                this.params.geometry = json;
                //this.params.geometry = JSON.stringify(json);
            }

            this.params.geometryType = processedInput.geometryType;

            function convertLatLng(coordinates, map) {
                for (var i = 0; i < coordinates.length; i++) {
                    var lng = coordinates[i][0];
                    var lat = coordinates[i][1];
                    var point = map.options.crs.project(L.latLng(lat, lng));
    ...
    EsriLeaflet.Layers.RasterLayer = L.Class.extend({
        ...
        onAdd: function (map) {
            ...
            var _mapBounds = this._map.getBounds();
            var ne = this._map.options.crs.project(_mapBounds._northEast);
            var sw = this._map.options.crs.project(_mapBounds._southWest);
            _mapBounds = L.bounds(L.point(sw.x, ne.y), L.point(ne.x, sw.y));
            if (this._currentImage && this._currentImage._projBounds.min.equals(_mapBounds.min) && this._currentImage._projBounds.max.equals(_mapBounds.max)) {
        ...
        _renderImage: function (url, bounds) {
            if (this._map) {
                ...
                /*
                *修改说明：调整图层顺序，添加缩放图层，新建的图层获取之前图层的zIndex
                *修改人：  陈永红
                *修改时间：2015/8/7
                *注释说明：注释代码为原来的代码（勿删）
                */
                ...
                */
                ...
                var bounds = L.bounds(L.point(sw.x, ne.y), L.point(ne.x, sw.y));

                var image;
                if (this._currentImage == undefined) {
                    //image = new L.ImageOverlay(url, bounds, {
                    image = new L.Proj.ImageOverlay(url, bounds, {
                        opacity: 0,
                        zIndex: -1
                    }).addTo(this._map);
                } else {
                    ...
                    image = new L.Proj.ImageOverlay(url, bounds, {
                        opacity: 0,
                        zIndex: this._currentImage._image.style.zIndex
                    }).addTo(this._map);
                }
                ...
                /*
                *修改说明：解决在IE调图时地图闪烁的问题
                *修改人：  陈永红
                *修改时间：2015/12/2
                *注释说明：添加一个时间差setTimeout，其他内容不变
                */
                //=============================================================
                ...
                image.once('load', function (e) {
                    ...
                    if (newImage._projBounds.min.equals(bounds.min) && newImage._projBounds.max.equals(bounds.max)) {
                        _this._currentImage = newImage;

                        if (_this.options.position === 'front') {
                            _this.bringToFront();
                        } else {
                            _this.bringToBack();
                        }

                        if (_this._map && _this._currentImage._map) {
                            _this._currentImage.setOpacity(_this.options.opacity);
                        } else {
                            _this._currentImage._map.removeLayer(_this._currentImage);
                        }

                        if (oldImage && _this._map) {
                            _this._map.removeLayer(oldImage);
                        }
                        ...
                        _this._map.removeLayer(newImage);
                    }
                    //}, 20);
    ...
    EsriLeaflet.Layers.DynamicMapLayer = EsriLeaflet.Layers.RasterLayer.extend({
        ...
        options: {
            ...
            format: 'png24',
            transparent: true,
            declaredClass: 'L.esri.Layers.DynamicMapLayer'
        },
        ...
        initialize: function (url, options) {
            ...
            options ? options.declaredClass = "L.esri.Layers.DynamicMapLayer" : '';
            L.Util.setOptions(this, options);
        },
        ...
        _buildExportParams: function () {
            ...
            var params = {
                ...
                /*
                *修改说明：如果地图没有地理坐标信息，需要删除该两个参数才能显示出地图
                *修改人：  陈永红
                *修改时间：2015/8/7
                *注释说明：注释为原有代码（勿删）
                */
                bboxSR: this.options.bboxSR ? this.options.bboxSR : null,
                imageSR: this.options.imageSR ? this.options.imageSR : null,
        ...
        _requestExport: function (params, bounds) {
            if (this.options.f === 'json') {
                ...
                var requestStr = this.url + 'export' + L.Util.getParamString(params);
                if (requestStr.length > 2000) {
                    params.f = 'json';
                    L.esri.Request.request("proxy/proxy.ashx?" + this.url + "export", params, function (r, b) {
                        this._renderImage(b.href, bounds);
                    }, this);
                } else {
                    this._renderImage(requestStr, bounds);
                }
            }
        }
    });
    ...
    EsriLeaflet.Layers.TiledMapLayer = L.TileLayer.extend({
        initialize: function (url, options) {
            options ? options.declaredClass = 'L.esri.Layers.TiledMapLayer' : "";
    ...
    (function (EsriLeaflet) {
        ...
        EsriLeaflet.Layers.FeatureManager = EsriLeaflet.Layers.FeatureGrid.extend({
            ...
            _buildQuery: function (bounds) {
                ...
                var query = this._service.query().where(this.options.where).fields(this.options.fields).precision(this.options.precision);
    ...
    EsriLeaflet.Layers.FeatureLayer = EsriLeaflet.Layers.FeatureManager.extend({
        ...
        //*重写方法，添加投影*//
        createNewLayer: function (geojson) {
            this.__project(geojson, this._map.options["crs"]);
            return L.GeoJSON.geometryToLayer(geojson, this.options.pointToLayer, L.GeoJSON.coordsToLatLng, this.options);
        },
        /*新加的方法*/
        __project: function (geoJosn, crs) {
            var _coordinates = geoJosn.geometry.coordinates;
            var array = [];
            if (geoJosn.geometry.type == "LineString") {
                for (var j = 0, coord; coord = _coordinates[j++];) {
                    var latlng = crs.projection.unproject(L.point(coord[0], coord[1]));
                    array.push([latlng.lng, latlng.lat]);
                }
            } else if (geoJosn.geometry.type == "Polygon") {
                for (var i = 0, coords; coords = _coordinates[i++];) {
                    var _array = [];
                    for (var j = 0, coord; coord = coords[j++];) {
                        var latlng = crs.projection.unproject(L.point(coord[0], coord[1]));
                        _array.push([latlng.lng, latlng.lat]);
                    }
                    array.push(_array);
                }
            } else if (geoJosn.geometry.type == "Point") {
                var latlng = crs.projection.unproject(L.point(coord[0], coord[1]));
                array.push(latlng.lng);
                array.push(latlng.lat);
            }
            geoJosn.geometry.coordinates = array;
            return geoJosn;
        },
