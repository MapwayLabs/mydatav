(function($){
    ...
    scrollTable.prototype = {
        ...
        init: function(userSetting) {
            ...
            // th对齐td
            this.alignOnResize($('#' + scrollData.setting.scrollId + ' .' + scrollData.setting.scrollBodyCls), $('#' + scrollData.setting.scrollId + ' .' + scrollData.setting.scrollHeaderCls));
            // 滚动  数据大于图表大小时，才进行滚动
          if (this.$elem.height() < $('#' + scrollData.setting.scrollId).height()) {
            	$('#' + scrollData.setting.scrollId + ' .' + scrollData.setting.scrollWrapCls).scrollTableTextSlider(scrollData.setting);
            }
        },
        // 表头数据处理
        ...
        getChartHtml: function() {
            var html = '<div id="<#= setting.scrollId #>">\
                    <div class="<#= setting.scrollWrapCls #> bdp-table-scroll-wrap">\
                        <div class="bdp-table-text-layer <#= setting.textLayerCls #>"></div>\
        ...
        getStyle: function(setting) {
            return '<style>\
                .bdp-table-text-layer {\
                    ...
                    background: #2E324D; \
                    text-align: left; \
                    box-shadow: 0 0 6px 0 rgba(0,0,0,.1), 0 12px 12px 0 rgba(0,0,0,.3);\
        		} \
				...
				.bdp-table-scroll tr {\
                    display: block;\
                    margin: 4px 2px;\
                    background: rgba(55,184,233,0.10);\
					border: 1px solid rgba(55,184,233,0.50);\
                }\
				.bdp-table-scroll tr:before {\
					content: "";\
                    position: absolute;\
                    width: 3px;\
                    height: 10px;\
    ...
    $.fn.scrollTableTextSlider.scllor = function($this, settings){
        ...
        var _tdTextDom = $( ('.' + settings.trCls + ' .text-body'), $this );
      	var _textLayer = $( ('.' + settings.textLayerCls + ":eq(0)"), $this );
        ...
        var autoPlay=function(e){
            toggleTextLayer(e, 'hide');
            timerID = window.setInterval(scrollUp,settings.timer);
        };
        var autoStop = function(e){
            window.clearInterval(timerID);
        };
      	var toggleTextLayer = function(e, type){
          if( !e || type == 'hide' ){
           _textLayer.hide().text('');
            return false;
          }
          _textLayer.show();
          var $tarDom = $(e.target);
          if( !$tarDom.hasClass('text-body') ){
            $tarDom = $(e.target).find('.text-body');
          }
          var textStr = ($tarDom.text() || '').trim();
          _textLayer.text(textStr);
          _textLayer.css('top', $tarDom.position().top + 24);
        ...
        _btnDown.hover(autoStop,autoPlay);
      	_tdTextDom.css("cursor","pointer").hover(toggleTextLayer);
    };
