...
}[window.EC_DEMO_LANG], configs = {}, lnTools = ace.require("ace/ext/language_tools"), keywords = {
    grid: 108210,
    series: 1161721,
    pie: 173151,
    legend: 278319,
...
_.each((location.search || "").substr(1).split("&"), function(e) {
    var t = e.split("=");
    configs[t[0]] = t[1]
});
var gb = {
...
function initEventHandler() {
    gb.enableLive && gb.editor.on("change", function() {
        runDebounce()
    }),
    ...
    $(window).mousemove(function(e) {
        gb.handler.isDown && setSplitPosition(e.clientX / window.innerWidth)
    }).mouseup(function() {
        gb.handler.isDown = !1
    }),
...
function setSplitPosition(e) {
    var t = 100 * (e = Math.min(.9, Math.max(.1, e)));
...
function checkEditorIfToShow() {
    window.innerWidth < 768 ? void 0 !== gb.editorIsShown && !0 !== gb.editorIsShown || ($("#code-container").hide(),
    $("#h-handler").hide(),
    $(".right-container").css("width", "100%").css("left", "0%"),
    gb.editorIsShown = !1) : void 0 !== gb.editorIsShown && !1 !== gb.editorIsShown || ($("#code-container").show(),
    $("#h-handler").show(),
    setSplitPosition(.4),
    gb.editorIsShown = !0)
}
...
$("#theme a").popover({
    html: !0,
    content: function() {
        var e = $(this).attr("class").replace("selected", "").trim();
        return '<div class="theme-palette ' + e + '">' + COLORS[e].map(function(e) {
            return '<span style="background-color:' + e + '"></span>'
        }).join("") + "</div>"
    },
    placement: "top",
    trigger: "hover"
...
var appEnv = {}, gui, _intervalIdList = [], _timeoutIdList = [], _oldSetTimeout = window.setTimeout, _oldSetInterval = window.setInterval;
function _clearTimeTickers() {
    for (var e = 0; e < _intervalIdList.length; e++)
        clearInterval(_intervalIdList[e]);
    for (e = 0; e < _timeoutIdList.length; e++)
        clearTimeout(_timeoutIdList[e]);
    _intervalIdList = [],
    _timeoutIdList = []
}
...
var run = function(ignoreOptionNotChange) {
    if (gb.chart || (gb.chart = echarts.init($("#chart-panel")[0], configs.theme, {
    ...
    hasEditorError())
        ...
        try {
            ...
            option && "object" == typeof option && (!_.isEqual(option, gb.lastOption) || ignoreOptionNotChange)) {
                gb.lastOption = option;
                var startTime = +new Date;
                gb.chart.setOption(option, !0);
                var endTime = +new Date;
                gb.updateTime = endTime - startTime;
                for (var debounceTime = 500, debounceTimeQuantities = [500, 2e3, 5e3, 1e4], i = debounceTimeQuantities.length - 1; 0 <= i; i--) {
            ...
            app.config) {
                ...
                for (var name in app.config) {
                    ...
                    if ("onChange" !== name && "onFinishChange" !== name) {
                        ...
                        if (configParameters[name] && (configParameters[name].options ? controller = gui.add(app.config, name, configParameters[name].options) : null != configParameters[name].min && (controller = gui.add(app.config, name, configParameters[name].min, configParameters[name].max))),
                        "string" == typeof obj)
                            try {
...
function localReset() {
    gb.loadedCode && (gb.editor.setValue(gb.loadedCode),
    gb.enableLive || runDebounce(),
    localSave(),
    $("#reset-btn").hide(),
    run())
...
function hasEditorError() {
    for (var e = gb.editor.getSession().getAnnotations(), t = 0, o = e.length; t < o; ++t)
        if ("error" === e[t].type)
            return !0;
    return !1
}
function load() {
    ...
    if (configs.gl) {
        ...
        o();
    function o() {
        configs.c && (console.log(configs.c),
        $.ajax("../" + e + "/" + configs.c + ".js", {
            dataType: "text",
            success: function(e) {
                gb.loadedCode = e,
...
function formatTime(e) {
    for (var t = [e.getHours(), e.getMinutes(), e.getSeconds()], o = "", n = 0, a = t.length; n < a; ++n)
        o += (t[n] < 10 ? "0" : "") + t[n],
        n < a - 1 && (o += ":");
    return o
}
